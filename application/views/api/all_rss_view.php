...
<rss version="2.0"
    ...
    <?php foreach($query -> result() as $item): ?>
        ...
        <item>
          ...
          <link><?php echo site_url('detail/record/' . $item -> id) ?></link>
          <guid><?php echo site_url('detail/record/' . $item -> id) ?></guid>

        <description><![CDATA[<?= $item -> description; ?>]]></description>
        <dc:description><![CDATA[<?= $item -> description; ?>]]></dc:description>
		<dc:rights>http://creativecommons.org/licenses/by-nc-sa/3.0/deed.en_GB</dc:rights>
		<dc:subject>ukoer</dc:subject>
		<?php
			$subjects = $this -> ResourceDB_model -> get_resource_subjects($item -> id);
			foreach ($subjects -> result() as $subject)
			{
				echo "<dc:subject>" . $subject -> title . "</dc:subject>\n";
			
			}
		
		?>
      <pubDate><?= rss_date($item -> metadata_modified); ?></pubDate>
        </item>
